...
- default assistant repo = `~/Projects/research-hub/research-assistant/`
- default jobs root = `~/Projects/research-hub/jobs/`
- each job remains one independent repo per research case

Those paths are defaults. `jobs_root` is now configurable through `config/paths.yaml`. `jobs-index/` remains fixed inside the assistant repo.
...
The runner now also persists run-level usage telemetry under `audit/usage/`. Execution-stage attempts, structured substages, and post-processing steps are tracked separately from qualification probes so workflow cost tuning is not polluted by provider-preflight overhead. Token counts are opportunistic exact telemetry only: when a CLI does not expose usage, records remain explicit with `usage_status: unavailable` rather than silently estimating or fabricating totals. Resume fast-paths do not append synthetic post-processing usage for already-existing claim-extraction or final-artifact outputs, and sibling-interrupted structured substeps are recorded as `cancelled` rather than misclassified as failures. Each run now also saves `audit/execution-config.json`, which snapshots the execution-relevant configuration for that specific run: the job-derived execution config when present, otherwise the fallback CLI adapter split, plus configured and resolved stage-provider/model assignments. The same artifact now also records `actual_stage_assignments`, so manual launches, per-step overrides, failed attempts, and cancelled attempts preserve which provider/model was actually tried. Re-entering an existing run under a different resolved execution config is now rejected instead of silently mixing provider selections inside one run, but unrelated non-execution edits elsewhere in `config.yaml` do not trip that guard.
