...
- `research-template.md` returns fixed markdown sections with numbered fact and inference claims, explicit confidence labels, and a source-evaluation section. It now also carries a per-pass role mandate (pass A weights established solutions, pass B weights the recent and announced solution horizon), candidate coverage rules that require enumerating the option space with maturity labels (`ga`, `recent_release`, `announced_unreleased`, `roadmap_signal`), a `# Candidate Landscape` section with a structured `candidates` payload for option-selection research, and independence rules forbidding reads of the parallel pass or other runs.
- `critique-template.md` returns fixed markdown sections that explicitly attack unsupported claims, weak sources, omissions, and overreach. Omission review now explicitly includes candidate-coverage failures: missing recently released or credibly announced alternatives.
- `judge-template.md` returns fixed markdown sections that separate supported conclusions from synthesis inferences, preserve unresolved disagreement when evidence is mixed, and may add a `# Brief Improvement Recommendations` section when the requester could materially improve the outcome by clarifying missing inputs. When candidate options are adjudicated, maturity must be weighed explicitly and availability risk for unreleased options stays inline.
