...
- run-level execution snapshots under `runs/<run-id>/audit/execution-config.json`, so provider/model selection, configured-vs-resolved stage assignments, trust requirements, and runtime-policy reroutes are preserved per run
- existing runs now reject resumed execution when the current resolved execution config no longer matches the saved run snapshot
