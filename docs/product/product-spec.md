...
- the freshness window is now read from the job config's `freshness.max_days` (with a 365-day default) and applied by the workflow runner, single-stage launcher, final-artifact generation, and final-artifact readiness validation
- research facts and inferences citing external evidence are now checked for quoted excerpts on their evidence support links: missing excerpts warn by default, and `requirements.require_evidence_excerpts: true` turns them into validation errors with the normal bounded-repair path
- each run now snapshots the exact `brief.md` and `config.yaml` it was scaffolded against under `runs/<run-id>/job-inputs/`; execution refuses to continue a run whose brief has since changed (the packets embed the original brief), and config drift is journaled as a warning while execution-relevant config drift remains enforced by the execution-config snapshot guard
- the workflow runner is now split into focused modules: `_execution_plan.py` (stage plan and run paths), `_workflow_prompts.py` (stage/substep/repair prompt builders), `_execution_guards.py` (protected-input snapshots, manifest refresh, job-input pinning), and `_run_state.py` (state transitions), with `execute_workflow.py` retaining orchestration
...
- the final report step only trusts output produced during the current invocation (a leftover report from an earlier run is never re-validated as fresh, and a file the adapter wrote wins over stdout chatter), runs inside the same protected-input snapshot/restore guard as the agent stages, and its usage telemetry estimates from the real synthesis prompt rather than the wrapper command
- the final report step is now individually tracked and individually routable: it has its own `post_processing.final_report` entry in `workflow-state.json` (distinct from the deterministic `final_artifact` entry, so a run interrupted between them resumes correctly instead of conflating the two), it may be pinned to its own provider/model via `workflow.execution.stage_providers.final-report` (falling back to the judge stage's assignment when omitted), and its actual provider/model assignment is recorded like every other stage, with its completed/failed outcome feeding the provider scorecards that runtime quarantine policy reads; a validated report is published to `outputs/final-report-<run-id>.md` alongside the deterministic artifact, in addition to its canonical copy at `runs/<run-id>/stage-outputs/07-final-report.md`
...
- the quality policy now supports `single_source_recommendation`, which fails publication when a recommendation rests on one evidence source without an explicit single-source acknowledgment; the judge contract marks recommendations with `claim_class: "recommendation"` and the acknowledgment (`single_source_acknowledged` on the judge item) survives claim-map construction, so agents can satisfy the gate without operator edits
- disagreements are now first-class run artifacts: critique unresolved disagreements carry stable `DIS-###` ids (agent-assigned per the templates, runner-assigned when missing), the runner merges them into `runs/<run-id>/disagreements.json`, judge validation flags any id the judge never addresses (warning by default, error under `requirements.require_disagreement_coverage`), and the register records each id's disposition after the judge: unresolved_by_judge, addressed, or unaddressed
- `check_source_links.py --verify-excerpts` fetches cited documents and verifies that quoted research-stage evidence excerpts actually appear in the source text (verified/partial/not_found), catching paraphrased or invented quotes after a run
- `verify_claim_entailment.py` runs sampled, deterministic LLM entailment spot-checks (opt-in by invocation, since it incurs provider cost): an adapter judges whether each quoted excerpt actually supports the claim citing it, and unsupported verdicts fail the audit
...
4. Source identity is now modeled, but source governance is still shallow.
   A run-level `sources.json` exists, source IDs must resolve, source records carry policy metadata, freshness now classifies dated sources as fresh or stale against the per-job `freshness.max_days` window, evidence excerpts are checked (and enforceable) on research stages, and a standalone link checker can verify locator reachability after a run. The job's `source_policy` lists are now enforced: matches against source type/authority escalate the policy outcome (disallowed → blocked at publication, caution → warning, preferred → annotated) and set an authority tier on the record. Semantic support is now auditable in two layers: excerpt-presence verification (deterministic) and sampled LLM entailment spot-checks (opt-in). The remaining gap is topic-level conflict policy across sources.
